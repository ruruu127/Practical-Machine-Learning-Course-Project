...
The accuracy of the model is 75.41%, which is okay.
...
This time, the model records a very high accuracy, which is 99.39%. But we shouldn't be too happy with this result as there may be the problem of overfitting in this model.
...
The third model also has a high accuracy (96.7%).

## Final Results
Based on the three models that we have created, we see that the random forest model records the highest percentage of accuracy. 

```{r echo=TRUE}
accuracy <- data.frame(cm.tree$overall[1], cm.forest$overall[1], cm.gbm$overall[1])
colnames(accuracy) <- c("Classification.Tree", "Random.Forest", "GBM")
accuracy
```
